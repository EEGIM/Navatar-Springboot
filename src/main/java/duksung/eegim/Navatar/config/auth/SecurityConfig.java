...
public class SecurityConfig extends WebSecurityConfigurerAdapter {
    ...
    protected void configure(HttpSecurity http) throws Exception {
        http.csrf().disable()
                ...
                .and()
                .authorizeRequests()
                .antMatchers("/", "/css/**", "/img/**", "/js/**", "/html/**", "/products/**", "/brands/**").permitAll() // antMatchers() - 권한 관리 대상 지정 옵션 - 쇼핑몰 상품 조회로 바꾸기..
                .antMatchers("/udasgdw").hasRole(Role.USER.name()) // 이렇게 하면, 로그인 해야 다른 메뉴들 사용 가능, 그리고 user 관련 창은 사용 불가 위에 permitall로 권한 옮겨 보기
