...
public interface SatisfactionRepository extends JpaRepository<Satisfaction, Long> {
    ...
    @Query(value = "select satisfaction sizeSatisfaction, count(s.satisfaction) count, size from Satisfaction s where s.Product_productNo=:productNo group by s.satisfaction, size order by count(s.satisfaction) desc;", nativeQuery = true)
    List<ProductSatisfactionDto> getProductSatisfaction(@Param("productNo") Long productNo);

    @Query(value = "select satisfaction sizeSatisfaction, count(s.satisfaction) count, size from Satisfaction s where s.Product_productNo=:productNo AND height between :height-3 and :height+3 AND weight between :weight-3 and :weight+3 group by s.satisfaction, size order by count(s.satisfaction) desc;", nativeQuery = true)
    List<ProductSatisfactionDto> getProductSatisfactionByUserSize(@Param("productNo") Long productNo, @Param("height") Long height, @Param("weight") Long weight);
//
